...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/6a19aa3130379bed5c4b10fc-preview") ? value : (value.startsWith("/") ? "/6a19aa3130379bed5c4b10fc-preview" + value : "/6a19aa3130379bed5c4b10fc-preview/" + value);
  };
...
const getVrioCampaignInfoBasedOnPaymentMethod = (isVipUpsell) => {
    const vrioCampaigns = [{"_id":"6a19aa0830379bed5c4b10e1","integration":[{"_id":"685435949a3a8c5ffb4854ef","workspace":"develop","platform":"vrio","description":"dev, team api","fields":{"publicApiKey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..z4qwr2v87T3wq73w1nT8aSASKIMVLnL0HX1E-2tavrs"},"status":"active","createdAt":1750335264215,"updatedAt":1750349204667,"__v":0,"category":"CRM","id":"685435949a3a8c5ffb4854ef"}],"externalId":"39","name":"uk","currency":"GBP","countries":[222],"metadata":{"campaign_id":39,"campaign_name":"","payment_type_id":1,"campaign_active":true,"campaign_prepaid":true,"campaign_payment_method_required":true,"campaign_group_transactions":true,"campaign_global_js":"","campaign_global_seo_title":"","campaign_global_seo_keywords":"","campaign_global_seo_description":"","date_created":"2026-05-29 15:00:25","created_by":0,"date_modified":"2026-05-29 15:00:25","modified_by":0,"campaign_notes":"","offers":[],"shipping_profiles":[],"campaignId":"39","externalId":39,"description":"","payment_methods":["amex","discover","visa","master"],"alternative_payments":[],"countries":[{"iso_numeric":826,"calling_code":"44","id":222,"name":"United Kingdom of Great Britain and Northern Ireland (the)","iso_2":"GB","iso_3":"GBR"}]},"funnels":[],"createdAt":1780046356416,"updatedAt":1780066825504,"packages":[],"status":"active","platform":"vrio","__v":0,"id":"6a19aa0830379bed5c4b10e1"}];
...
function setActiveTestId(el, testId) {
  if (!el) return;
  if (el.dataset.originalTestid === undefined) {
    el.dataset.originalTestid = el.getAttribute("data-testid") || "";
  }
  el.setAttribute("data-testid", testId);
}

function restoreTestId(el) {
  if (!el) return;
  if (el.dataset.originalTestid === undefined) return;
  const original = el.dataset.originalTestid;
  original ? el.setAttribute("data-testid", original) : el.removeAttribute("data-testid");
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "9Hvn-lUF38XdDS_b7i9zPlVMv6WxoNBdJBsmB5vo2D91cZyvZKO-oo8HXLkSi0LA",
    action: "process",
...
if (selectedProductEl) {
  ...
  setActiveTestId(selectedProductEl, "active-bundle");
  if (variantsBox) {
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/6a19aa3130379bed5c4b10fc-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
...
if (typeof validateAndSendToKlaviyo === "function") {
  ...
  function setClickBumpOptionActive(group, optionToSelect) {
    group.querySelectorAll("[data-cb-option]").forEach((o) => {
      const isActive = o === optionToSelect;
      o.classList.toggle("cb-option-active", isActive);
      isActive
        ? setActiveTestId(o, "active-click-bump-option")
        : restoreTestId(o);
    });
  ...
  productsElements.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      setActiveTestId(card, "active-bundle");
      if (variantsBox) {
        variantsBox.innerHTML = "";
...
const reconcileUpsellSelectForKlarna = (group, isKlarnaPayment) => {
    ...
    if (fallbackOption) {
      ...
      options.forEach((o) => restoreTestId(o));
